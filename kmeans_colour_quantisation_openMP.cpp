    ...
    for (int i = 0; i < numIterations; ++i) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            for (int j = 0; j < nClusters; ++j) {
                ...
                if (dist < minDist) {
                    minDist = dist;
                    closestCluster = j; // Assign the point to the nearest centroid
                }
            }
        ...
        #pragma omp parallel for
        for (int k = 0; k < n; ++k) {
            const ColorPoint& point = colors[k];
...
int main(int argc, char* argv[]) {
    ...
    t2 = omp_get_wtime();
	printf("Application: colour quantisation of image. Parallelisation: openMP. Data file: %s, Time: %.3f, numThreads: %d\n", 
		inputImageFileName.c_str(), t2 - t1, numThreads);
